...
auth_email="xxx@gmail.com"    #你的CloudFlare注册账户邮箱
auth_key="xxxxxxx"   #你的CloudFlare账户key,位置在域名概述页面点击右下角获取api key。
...
record_type="A"     
#获取zone_id、record_id
zone_identifier=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones?name=$zone_name" -H "X-Auth-Email: $auth_email" -H "X-Auth-Key: $auth_key" -H "Content-Type: application/json" | perl -nle 'print $& if /"id":"\K[^"]*/' | head -1 )
echo $zone_identifier

sed -n '2,20p' result.csv | while read line
do
    echo $record_name$record_count'.'$zone_name
    record_identifier=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$zone_identifier/dns_records?name=$record_name$record_count"'.'"$zone_name" -H "X-Auth-Email: $auth_email" -H "X-Auth-Key: $auth_key" -H "Content-Type: application/json" | perl -nle 'print $& if /"id":"\K[^"]*/' | head -1 )
    ...
    if [[ "$update" != "${update%success*}" ]] && [[ "$(echo $update | grep "\"success\":true")" != "" ]]; then
      echo $(date +%T%n%F) $record_name$record_count'.'$zone_name'更新为:'${line%%,*}'....成功' >> ./success.log
    else
      echo $(date +%T%n%F) $record_name$record_count'.'$zone_name'更新失败:'$update >> ./fail.log
    fi
